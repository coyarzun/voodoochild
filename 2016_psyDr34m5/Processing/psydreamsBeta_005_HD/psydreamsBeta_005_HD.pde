//rediseñar interfaz, enable filters en glsl, invertir orden, etc. refinar
import ddf.minim.*;
import oscP5.*;
import netP5.*;
...
void setup(){
  if(fs)size(displayWidth, displayHeight, P3D);
  else  size(1920/1, 960/1, P3D);
  colorMode(HSB);
  textureWrap(REPEAT);
  ...
  buffer      = createGraphics(1920,960,P3D);//1024,512,P3D);//800, 400, P3D);//800, 400, P3D);
  ...
  String ip = "10.76.9.37";//"192.168.1.6";//169.254.61.66";//192.168.0.71";//"192.168.0.183"
...
void drawBuffer(){
  ...
  buffer.translate(buffer.width/2.0,buffer.height/2.0);
...
int     takeLength, takeCount, recordCount;
String  recRoot     = "../_render/";
String  recXtension = ".png";
String  recFolder, recFilename;

void recordStuff(){
  if(recording){
    String tmp = recRoot+recFolder+recFilename+"_"+nf(recordCount,3)+recXtension;
    save(tmp);
    println("Saving "+tmp+"...");
    recordCount++;
    if(recordCount>=takeLength)stopRecording();
  }
}
void startRecording(){
  recording   = true;
  ...
  takeCount++;
  takeLength  = 30*5;
  recFilename = "psyD_HD_"+nf(takeCount,3);
  recFolder   = recFilename+"/";
...
void keyPressed(){
  if(keyCode==32 && !recording) startRecording();
}
