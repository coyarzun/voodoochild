class FreshWeedow extends VZong {
  int[] drumpads = {
  ...
  FreshWeedow() {
    ...
    name = "Fresh Weedow";
    lyricsbase = loadStrings("data/freshwidow2010.txt");
    definePatterns();
    makeSong();
  }
  ...
  void makePattern(int drumIndex, int bassIndex, int bassNoteIndex, int kbIndex, int kbNoteIndex, boolean singa, boolean bells) {// 
    for (int i=0; i<16; i++) {
      ...
      if (!hxtep.equals("-")) {
        ...
        score.addNote(float(position+i)/4, 9, 40, pitx, 120+(int)random(7), 0.25, 0.8, 64);//random(127));//80);
        ...
        basstrack2.addEvent(new Note(pitx, 0, 0), auxposition+postpos);
      }
      String xhxtep = kbpatterns[kbIndex].substring(i, i+1);
      if (!xhxtep.equals("-")) {
        int xnote = 48+12+kbnotes[kbNoteIndex][i];
        int xpitx = xnote+unhex(xhxtep);
        score.addNote(float(position+i)/4, 3, 0, xpitx, 96+(int)random(7), 0.25*8, 0.8, 64);//random(127));//80);
      ...
      if (bells) {
        ...
        int zpitx = 0;
        if (i%2==0) {
          int oo = (i==0 || i==8 ) ? 0 : (i==2 || i==10) ? 2 : (i>8) ? (random(2)>1)? 2:1 : 
          3;
          zpitx = bellsnotes[oo];
          ypitx+=zpitx;
          score.addNote(float(position+i)/4, 1, 0, ypitx, 60+(int)random(2), 0.25*8, 0.8, 64);//random(127));//80);
